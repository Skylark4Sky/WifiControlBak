...
local function downloadfirmware(firmware)
	...
	while true do
		...
		fileSize = 0;
		download_path = nil;		
		http.request("GET",firmware.url,nil,nil,nil,8000,function (respond,statusCode,head,filePath) 
...
local function updateFirmware_Proc(update_ctr)
	while true do	
		...
		if result == true then
			while true do 
				local download_ok =	new_firmware_download_proc(update_ctr)
