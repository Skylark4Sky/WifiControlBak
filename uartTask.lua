...
--空闲
GISUNLINK_NETMANAGER_IDLE = 0x00 
--开始连接
GISUNLINK_NETMANAGER_START = 0x01
--连接中
GISUNLINK_NETMANAGER_CONNECTING = 0x02       
--已连接
GISUNLINK_NETMANAGER_CONNECTED = 0x03       
--断开连接
GISUNLINK_NETMANAGER_DISCONNECTED = 0x04 
--重连中
GISUNLINK_NETMANAGER_RECONNECTING = 0x05 
--进入配对
GISUNLINK_NETMANAGER_ENT_CONFIG = 0x06  
--退出配对
GISUNLINK_NETMANAGER_EXI_CONFIG = 0x07   
--保存配对信息
GISUNLINK_NETMANAGER_SAVE_CONFIG = 0x08   
--同步时钟成功
GISUNLINK_NETMANAGER_TIME_SUCCEED = 0x09     
--同步时钟失败
GISUNLINK_NETMANAGER_TIME_FAILED = 0x0A 
--已连上平台
...
--固件检查OK
GISUNLINK_FIRMWARE_CHK_OK = 0x89							
--固件检查不OK
